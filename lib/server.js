...
Server.prototype.middleware = function() {
	...
	return function(req, res, next) {
		...
		req.tasks = server.routes.filter(function(route) {
			return match(trim(req.path), route.glob)
		});
		...
		var sources = req.tasks.map(function(r) { return r.source; }).filter(unique).join(',');
		...
		// get source files
		var glob = new Glob(req.source, { cwd: server.root, nomount: true, nodir: true, stat: true });
		...
		fs.stat(req.cached, function(err, cached) {
			...
			// get stat for all 
			var stats = Object.keys(glob.statCache).map(function(k) { return glob.statCache[k]; });
		...
		function compile() {
			...
			process(req.source, function(err, file) {
		 ...
		 * @returns {Boolean}
		 */
		function process(src, callback) {
			
			var stack = compiler();
			
			gulp.src(src, { base: server.root, cwd: server.root })
				.pipe(stack())
				.on('error', error)
			...
			function error(e) {
				...
				err.filename = e.filename || e.file || req.source;
				err.plugin = e.plugin;
				callback(err);
			}
			
			function end() {
				callback(null, req.output);
			}
		...
		function compiler() {
			var result = lazypipe();
			req.tasks.forEach(attach);
			return req.tasks.length > 0 ? result : util.noop;
			function attach(route) {
				result = route.attach(result);
			}
		}

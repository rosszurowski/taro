...
Server.prototype.resolve = function(req, done) {
	...
	// source
	files = glob(req.tasks[0].source, { cwd: this.root, nomount: true, nodir: true });
	for (var i = 0, l = files.length; i < l; i++) {
		// check if there's a match anywhere in the string
		file = files[i];
...
Server.prototype.compiler = function(req) {
	var compiler, length = 0;
	compiler = lazypipe();
	this.tasks.forEach(function(task) {
		if (!match(req.source, task.pattern, { matchBase: true })) return;
		compiler = task.attach(compiler);
		length++;
	});
	return length > 0 ? compiler : util.noop;
}

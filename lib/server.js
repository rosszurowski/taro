...
/**
 * Return a stream plugin that combines all server and task plugins
 * @param {Request} req
 * @returns {Stream}
 */
Server.prototype.run = function(req) {
	var tasks;
	// plugins = this.plugins;
	tasks = this.tasks.filter(function(task) {
		return match(req.source, task.pattern, { matchBase: true });
	});
	// render lazypipe
	// var output = lazypipe();
	// tasks.forEach(function(task) {
	// 	output.pipe(task.stream);
	// });
	return util.noop();
}

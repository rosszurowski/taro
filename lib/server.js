...
Server.prototype.resolve = function(req, done) {
	...
	function search(next) {
		...
		glob(patterns.join(','), { cwd: server.root, nomount: true, nodir: true },
			function(err, files) {
				if (err) return next(err);
				if (!files.length) return next(new NotFoundError());
				files = files.filter(function(file) {
					normalized = trim(req.path.replace(path.extname(req.path), ''));
					return file.indexOf(normalized) !== -1;
				});
				source = files.length > 1 ? files : files[0];
				next();
			});
	}
...
Server.prototype.compile = function() {
	...
	return function(req, res, next) {
		...
		function handler(err) {
			...
			var error = new Error();
			error.status = 500;
			error.type = err.type || err.name || 'Error';
			error.message = err.message;
			error.lineNumber = err.lineNumber;
			error.filename = err.filename || err.file || req.source;
